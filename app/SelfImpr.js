...
const categories = [
  ...
  {
    title: "Books just dont offer practical",
    subtitle:
      "Remember when you learned to ride a bike just by reading? Yeah, we dont either.We help you take concrete small steps towards your goals. Learning just a bit, then immediately springing into action.",
  },
  {
...
const SelfImpr = () => {
  ...
  return (
    <Stack flexDirection="column" display="flex" height="180vh">
      <Stack flexDirection="column" m={10} marginLeft={15}>
        <span ref={text3Ref} style={{ fontSize: "20px" }}>
          Wrong with self-imporvement & how we're fixing it.
        </span>
        <span ref={text4Ref} style={{ fontSize: "60px", fontWeight: "bold" }}>
      ...
      <Box
        ...
        alignItems={"center"}
        p={2}
        sx={{ overflow: "auto", maxHeight: "40em" }}
      >
        <Stack
          direction={"row"}
          overflowY={"auto"}
        ...
        >
          {categories.map((category) => (
            ...
            >
              ...
              >
                {" "}
                <CircleIcon fontSize="small" /> {category.title}
              </span>
              <br></br>
              <span style={{ opacity: 1, color: "black ", fontSize: "18px" }}>
                {category.subtitle}
              </span>
            </Paper>
          ))}
      ...
      >
        <span style={{ fontWeight: "bold", fontSize: "40px" }}>
          Be the best you<br></br> with EQ{" "}
        </span>
        <span style={{ color: "grey", fontSize: "20px" }}>
          Not having your own emotions <br></br>under control might be holding
          <br></br> you back.{" "}
        </span>
        <span style={{ color: "grey", fontSize: "20px" }}>
          Additionally, not understanding<br></br> those of others stops you
          from <br></br>being parent, friend you can be.{" "}
        </span>
      </Stack>
    </Stack>
  );
